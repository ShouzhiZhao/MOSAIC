...
from .memory_backend import BehavioralMemory


class BehavioralAgent(GenerativeAgent):
    id: int
    ...
    memory: BaseMemory
    """The memory module in BehavioralAgent."""
    ...
    @classmethod
    def from_interactive_agent(cls, interactive_agent_instance: "BehavioralAgent"):
        new_instance = cls(
            id=interactive_agent_instance.id,
            name=interactive_agent_instance.name,
            age=interactive_agent_instance.age,
            gender=interactive_agent_instance.gender,
            traits=interactive_agent_instance.traits,
            status=interactive_agent_instance.status,
            interest=interactive_agent_instance.interest,
            relationships=interactive_agent_instance.relationships,
            feature=interactive_agent_instance.feature,
            llm=interactive_agent_instance.llm,
            memory=interactive_agent_instance.memory,
            event=interactive_agent_instance.event,
            avatar_url=interactive_agent_instance.avatar_url,
            idle_url=interactive_agent_instance.idle_url,
            watching_url=interactive_agent_instance.watching_url,
            chatting_url=interactive_agent_instance.chatting_url,
            posting_url=interactive_agent_instance.posting_url,
        )
        return new_instance

    def __lt__(self, other: "BehavioralAgent"):
...
"""
        ...
        kwargs: Dict[str, Any] = dict(
            ...
            select_prompt="You need to select **at least one** movie type from the listed interests."
            if not inverse_selection
            else "You are initially shown all the movies as your selected interests, however you may not be interested in all of them. You need to **deselect** the types you are not interested in.",
            select_instruction="Focus on choosing a small subset of interests that best match your personality and preferences."
            if not inverse_selection
            else "Review the initial selections and remove at least one that do not reflect your actual tastes. Please tell me the *removed* types.",
...
"""
    ...
    def get_summary(
        ...
        return f"Name: {self.name} (age: {age}) Interest: {self.interest}" + f"\n{result}"
    ...
    def _generate_reaction(
        ...
        kwargs: Dict[str, Any] = dict(
            ...
            agent_name=self.name,
            observation=observation,
            watched_history=(self.watched_history if len(self.watched_history) > 0 else "nothing"),
            heared_history=(self.heared_history if len(self.heared_history) > 0 else "nothing"),
    ...
    def get_memories_until_limit(self, consumed_tokens: int) -> str:
        ...
        retriever = (
            ...
            if type(self.memory) is BehavioralMemory
    ...
    def generate_plan(self, observation: str, now: Optional[datetime] = None) -> Tuple[bool, str]:
        call_to_action_template = (
    ...
    def take_action(self, now) -> Tuple[str, str]:
        ...
        self.memory.save_context(
            {},
            {
                self.memory.add_memory_key: f"{self.name} take action: {result}",
                self.memory.now_key: now,
            },
        )
    ...
    def generate_feeling(self, observation: str, now) -> str:
        ...
        self.memory.save_context(
            {},
            {
                self.memory.add_memory_key: f"{self.name} felt: {feelings}",
                self.memory.now_key: now,
            },
        )
        return feelings
    ...
    def update_watched_history(self, items, now=None):
        ...
        if any(not isinstance(item, str) or not item.strip() for item in items):
            raise ValueError("watched history requires nonempty movie names")
    ...
    def update_heared_history(self, items, now=None):
        ...
        with getattr(self.memory, "_memory_lock", nullcontext()):
