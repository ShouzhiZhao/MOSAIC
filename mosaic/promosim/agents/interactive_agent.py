"""Optional human-controlled PromoSim agent."""
...
class InteractiveAgent(BehavioralAgent):
    """
    InteractiveAgent is an extension class for `BehavioralAgent`, which mainly overwrites some methods, where we replace LLM with
    ...
    @classmethod
    def from_behavioral_agent(cls, behavioral_agent_instance: BehavioralAgent):
        new_instance = cls(
            id=behavioral_agent_instance.id,
            name=behavioral_agent_instance.name,
            age=behavioral_agent_instance.age,
            gender=behavioral_agent_instance.gender,
            traits=behavioral_agent_instance.traits,
            status=behavioral_agent_instance.status,
            interest=behavioral_agent_instance.interest,
            relationships=behavioral_agent_instance.relationships,
            feature=behavioral_agent_instance.feature,
            llm=behavioral_agent_instance.llm,
            memory=behavioral_agent_instance.memory,
            event=behavioral_agent_instance.event,
            avatar_url=behavioral_agent_instance.avatar_url,
            idle_url=behavioral_agent_instance.idle_url,
            watching_url=behavioral_agent_instance.watching_url,
            chatting_url=behavioral_agent_instance.chatting_url,
            posting_url=behavioral_agent_instance.posting_url,
            memory_retriever=behavioral_agent_instance.memory_retriever,
        )
        return new_instance
    ...
    def take_action(self, now) -> Tuple[str, str]:
        ...
        action = asyncio.run(
            self.get_response("You can input some text to explain your choice. \n")
        )
    ...
    def take_recommender_action(self, observation, now) -> Tuple[str, str]:
        ...
        if order == "1":
            ...
            items = asyncio.run(
                self.get_response("Please search single, specific item name want to search. \n")
            )
            # Construct the list of films with '<*>' format.
    ...
    def generate_feeling(self, observation: str, now) -> str:
        ...
        feeling = asyncio.run(
            self.get_response("Please input your feelings, which should be split by semicolon: \n")
        )
