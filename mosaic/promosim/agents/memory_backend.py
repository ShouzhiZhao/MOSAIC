"""Sensory, short-term, and long-term memory for PromoSim agents."""
...
from ..utils.concurrency import SerializableRLock, synchronized_memory

logger = logging.getLogger(__name__)


class BehavioralMemoryRetriever(TimeWeightedVectorStoreRetriever):
    """
    BehavioralMemoryRetriever is to retrieve memories from long-term memory module based on memory salience, importance and recency.
    """
    ...
    def get_relevant_documents(self, query: str) -> List[Document]:
        ...
        for doc, _ in rescored_docs:
            if (
                retrieved_num < self.k
                and doc.page_content.find("[FORGET]") == -1
...
class SensoryMemory:
    ...
    def __init__(self, llm, buffer_size=1):
        ...
        :param llm: The LLM object passed from BehavioralMemory.
    ...
    def dump_shortTerm_list(self):
        ...
        order_str = (
            "You should summarize the above observation(s) into one independent sentence."
    ...
    def add_ssm(self, obs):
        """
        This function is only called in the function BehavioralMemory.save_context(). It is used to transport observations to a piece of short term memory.
        ...
        # If the buffer is full, then dump and return the short term list to BehavioralMemory.
    ...
    def discard_memories(self) -> str:
        ...
        if len(self.short_memories) > self.capacity:
            ...
            for idx in range(len(self.short_memories) - 1):
                memory_dict[self.short_memories[idx]] = {
...
class LongTermMemory(BaseMemory):
    """
    Long-term memory is the memory base for the BehavioralAgent.
    """

    llm: BaseLanguageModel
    now: datetime
    memory_retriever: BehavioralMemoryRetriever
    ...
    def fetch_memories(self, observation: str, stm=None, now: Optional[datetime] = None):
        ...
        # retrieval enhance the short-term memories
            ...
            for document in retrieved_list:
                memory_content, memory_importance, insight_content = stm.add_stm_memory(
                    document.page_content, document.metadata["importance"], op="Retrieval"
                )
    ...
    def _get_insights_on_topic(self, topic: str, now: Optional[datetime] = None):
            ...
            for insight in insights:
                ...
                for par in par_list:
                    if par < len(self.memory_retriever.memory_stream):
                        importance_cur += self.memory_retriever.memory_stream[par].metadata[
                            "importance"
                        ]
                        valid += 1
                if valid == 0:
    ...
    def pause_to_forget(self):
        ...
        if len(prob_list) != 0:
            for idx in range(len(prob_list)):
                if (
                    self.now - self.memory_retriever.memory_stream[idx].metadata["last_accessed_at"]
    ...
    def get_memory(self) -> str:
        ...
        return " ".join(
            [re.sub(r"^\[\d+\]\s*", "", mem.page_content).strip() for mem in recent_memories]
        )


class BehavioralMemory(BaseMemory):
    """
    BehavioralMemory is the proposed memory module for BehavioralAgent. We replace `GenerativeAgentMemory` with this class.
    ...
    _memory_lock: Any = PrivateAttr(default_factory=SerializableRLock)
    ...
    def __init__(self, llm, memory_retriever, now, verbose=False, reflection_threshold=None):
        super(BehavioralMemory, self).__init__()
    ...
    def __setstate__(self, state):
        ...
        # Checkpoints made before synchronization was added have no lock.
        if not hasattr(self, "_memory_lock"):
            self._memory_lock = SerializableRLock()
    ...
    def load_memory_variables(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ltm_memory_list, memories_tuple = self.longTermMemory.fetch_memories_with_list(
            inputs["observation"], self.shortTermMemory
        )
    ...
    def save_context_after_retrieval(self, memories_tuple):
        """
        The BehavioralAgent can transfer short-term memory to long-term memory.
        ...
        insight_scores_list = [
            self._score_memory_importance(memory) for memory in insight_memory_list
        ]

        all_memories = ltm_memory_list + insight_memory_list
        all_memory_scores = ltm_importance_scores + insight_scores_list
        save_ltm_memory = [
            (all_memory_scores[i], self.now, all_memories[i]) for i in range(len(all_memories))
        ]
    ...
    def save_context(self, inputs: Dict[str, Any], outputs: Dict[str, str]) -> None:
        """
        The BehavioralAgent can add memory by using this function.
        :param inputs: Will be directly pass to LongTermMemory. [No use for this version.]
        :param outputs: The core memory dict that is passed from BehavioralAgent. It has to obtain the key 'add_memory' to save the memory content.
        :return: None
        """
    ...
    def clear(self) -> None:
        """
        Clear all the (long term) memory in BehavioralMemory.
        """
